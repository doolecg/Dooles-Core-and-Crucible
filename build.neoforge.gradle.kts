...
neoForge {
    ...
    runs {
        register("client") {
            ...
            client()
            jvmArgument("-Ddooles_core_crucible.debug.audit=${findProperty("mixinAudit") ?: "false"}")
...
tasks {
    ...
    processResources {
        ...
        val props = buildMap {
            ...
            put("at", at)
            // NeoForge 26.x deprecates logoFile in favour of iconFile
            val logoKey = if (sc.current.parsed >= "1.21.2") "iconFile" else "logoFile"
            inputs.property("logo_key", logoKey)
            put("logo_key", logoKey)
    ...
    val collectJar = register("collectJar") {
        ...
        val target = rootProject.layout.projectDirectory.dir("versions").file(jar.flatMap { it.archiveFileName })
        inputs.file(source)
        outputs.file(target)
