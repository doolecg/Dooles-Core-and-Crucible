...
/**
 * A tiered bow. Arrows leave it with the tier's damage multiplier and armor pierce ({@link LauncherShots});
 * the faster draw is applied to every bow by {@code BowItemMixin}.
 */
public class TieredBowItem extends BowItem {

    private final ModTiers tier;

    public TieredBowItem(ModTiers tier, Properties properties) {
        super(properties);
        this.tier = tier;
    }

    @Override
    // Vanilla makes the arrow, then LauncherShots applies this tier's damage multiplier and armor pierce.
